<?xml version="1.0" encoding="utf-8"?>
<wfs:FeatureCollection
  xmlns="http://skjema.geonorge.no/SOSITEST/produktspesifikasjon/FKB-Høydekurve/5.0"
  xmlns:wfs="http://www.opengis.net/wfs/2.0"
  xmlns:gml="http://www.opengis.net/gml/3.2"
  xmlns:xlink="http://www.w3.org/1999/xlink"
  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  xsi:schemaLocation="http://skjema.geonorge.no/SOSITEST/produktspesifikasjon/FKB-Høydekurve/5.0
                     http://skjema.geonorge.no/SOSITEST/produktspesifikasjon/FKB-Høydekurve/5.0/FKBHøydekurve.xsd
                     http://www.opengis.net/wfs/2.0
                     http://schemas.opengis.net/wfs/2.0/wfs.xsd"
  timeStamp="2021-09-29T15:06:04Z"
  numberMatched="unknown"
  numberReturned="0">
  <wfs:member>
    <Høydekurve gml:id="Høydekurve.1">
      <identifikasjon>
        <Identifikasjon>
          <lokalId>Høydekurve.1</lokalId>
          <navnerom>http://data.geonorge.no/SOSI/FKBHøydekurve</navnerom>
          <versjonId>Some meaningful text</versjonId>
        </Identifikasjon>
      </identifikasjon>
      <oppdateringsdato>2021-07-17T19:25:00Z</oppdateringsdato>
      <datafangstdato>2021-07-17</datafangstdato>
      <medium>T</medium>
      <verifiseringsdato>2021-07-17</verifiseringsdato>
      <registreringsversjon>2014-03-01</registreringsversjon>
      <informasjon>Some meaningful text</informasjon>
      <kvalitet>
        <Posisjonskvalitet>
          <datafangstmetode>2</datafangstmetode>
          <nøyaktighet>42</nøyaktighet>
          <synbarhet>2</synbarhet>
          <datafangstmetodeHøyde>2</datafangstmetodeHøyde>
          <nøyaktighetHøyde>42</nøyaktighetHøyde>
        </Posisjonskvalitet>
      </kvalitet>
      <senterlinje>
        <gml:LineString srsName="http://www.opengis.net/def/crs/epsg/0/5972" srsDimension="3">
          <gml:posList>568525,84 6661998,97 90,8 568520,52 6662017,57 90,8 568517,55 6662029,69 90,8 568511,79 6662034,72 90,8</gml:posList>
        </gml:LineString>
      </senterlinje>
      <høyde>21.21</høyde>
      <opphav>
        <Opphav>
          <sikkerTerrenggjengivelse>true</sikkerTerrenggjengivelse>
          <datakilde>GenererteDataFraBildematching</datakilde>
        </Opphav>
      </opphav>
    </Høydekurve>
  </wfs:member>
  <wfs:member>
    <Forsenkningskurve gml:id="Forsenkningskurve.1">
      <identifikasjon>
        <Identifikasjon>
          <lokalId>Forsenkningskurve.1</lokalId>
          <navnerom>http://data.geonorge.no/SOSI/FKBHøydekurve</navnerom>
          <versjonId>Some meaningful text</versjonId>
        </Identifikasjon>
      </identifikasjon>
      <oppdateringsdato>2021-07-17T19:25:00Z</oppdateringsdato>
      <datafangstdato>2021-07-17</datafangstdato>
      <medium>T</medium>
      <verifiseringsdato>2021-07-17</verifiseringsdato>
      <registreringsversjon>2014-03-01</registreringsversjon>
      <informasjon>Some meaningful text</informasjon>
      <kvalitet>
        <Posisjonskvalitet>
          <datafangstmetode>2</datafangstmetode>
          <nøyaktighet>42</nøyaktighet>
          <synbarhet>2</synbarhet>
          <datafangstmetodeHøyde>2</datafangstmetodeHøyde>
          <nøyaktighetHøyde>42</nøyaktighetHøyde>
        </Posisjonskvalitet>
      </kvalitet>
      <senterlinje>
        <gml:LineString srsName="http://www.opengis.net/def/crs/epsg/0/5972" srsDimension="3">
          <gml:posList>568625,84 6661998,97 90,8 568620,52 6662017,57 90,8 568617,55 6662029,69 90,8 568611,79 6662034,72 90,8</gml:posList>
        </gml:LineString>
      </senterlinje>
      <høyde>21.21</høyde>
      <opphav>
        <Opphav>
          <sikkerTerrenggjengivelse>true</sikkerTerrenggjengivelse>
          <datakilde>GenererteDataFraBildematching</datakilde>
        </Opphav>
      </opphav>
    </Forsenkningskurve>
  </wfs:member>
  <wfs:member>
    <Toppunkt gml:id="Toppunkt.1">
      <identifikasjon>
        <Identifikasjon>
          <lokalId>Toppunkt.1</lokalId>
          <navnerom>http://data.geonorge.no/SOSI/FKBHøydekurve</navnerom>
          <versjonId>Some meaningful text</versjonId>
        </Identifikasjon>
      </identifikasjon>
      <oppdateringsdato>2021-07-17T19:25:00Z</oppdateringsdato>
      <datafangstdato>2021-07-17</datafangstdato>
      <medium>T</medium>
      <verifiseringsdato>2021-07-17</verifiseringsdato>
      <registreringsversjon>2014-03-01</registreringsversjon>
      <informasjon>Some meaningful text</informasjon>
      <kvalitet>
        <Posisjonskvalitet>
          <datafangstmetode>2</datafangstmetode>
          <nøyaktighet>42</nøyaktighet>
          <synbarhet>2</synbarhet>
          <datafangstmetodeHøyde>2</datafangstmetodeHøyde>
          <nøyaktighetHøyde>42</nøyaktighetHøyde>
        </Posisjonskvalitet>
      </kvalitet>
      <posisjon>
        <gml:Point srsName="http://www.opengis.net/def/crs/epsg/0/5972" srsDimension="3">
          <gml:pos>568658,83 6662075,36 90,67</gml:pos>
        </gml:Point>
      </posisjon>
      <høyde>21.21</høyde>
      <opphav>
        <Opphav>
          <sikkerTerrenggjengivelse>true</sikkerTerrenggjengivelse>
          <datakilde>GenererteDataFraBildematching</datakilde>
        </Opphav>
      </opphav>
    </Toppunkt>
  </wfs:member>
  <wfs:member>
    <Forsenkningspunkt gml:id="Forsenkningspunkt.1">
      <identifikasjon>
        <Identifikasjon>
          <lokalId>Forsenkningspunkt.1</lokalId>
          <navnerom>http://data.geonorge.no/SOSI/FKBHøydekurve</navnerom>
          <versjonId>Some meaningful text</versjonId>
        </Identifikasjon>
      </identifikasjon>
      <oppdateringsdato>2021-07-17T19:25:00Z</oppdateringsdato>
      <datafangstdato>2021-07-17</datafangstdato>
      <medium>T</medium>
      <verifiseringsdato>2021-07-17</verifiseringsdato>
      <registreringsversjon>2014-03-01</registreringsversjon>
      <informasjon>Some meaningful text</informasjon>
      <kvalitet>
        <Posisjonskvalitet>
          <datafangstmetode>2</datafangstmetode>
          <nøyaktighet>42</nøyaktighet>
          <synbarhet>2</synbarhet>
          <datafangstmetodeHøyde>2</datafangstmetodeHøyde>
          <nøyaktighetHøyde>42</nøyaktighetHøyde>
        </Posisjonskvalitet>
      </kvalitet>
      <posisjon>
        <gml:Point srsName="http://www.opengis.net/def/crs/epsg/0/5972" srsDimension="3">
          <gml:pos>568758,83 6662075,36 90,67</gml:pos>
        </gml:Point>
      </posisjon>
      <høyde>21.21</høyde>
      <opphav>
        <Opphav>
          <sikkerTerrenggjengivelse>true</sikkerTerrenggjengivelse>
          <datakilde>GenererteDataFraBildematching</datakilde>
        </Opphav>
      </opphav>
    </Forsenkningspunkt>
  </wfs:member>
  <wfs:member>
    <Terrenglinje gml:id="Terrenglinje.1">
      <identifikasjon>
        <Identifikasjon>
          <lokalId>Terrenglinje.1</lokalId>
          <navnerom>http://data.geonorge.no/SOSI/FKBHøydekurve</navnerom>
          <versjonId>Some meaningful text</versjonId>
        </Identifikasjon>
      </identifikasjon>
      <oppdateringsdato>2021-07-17T19:25:00Z</oppdateringsdato>
      <datafangstdato>2021-07-17</datafangstdato>
      <medium>T</medium>
      <verifiseringsdato>2021-07-17</verifiseringsdato>
      <registreringsversjon>2014-03-01</registreringsversjon>
      <informasjon>Some meaningful text</informasjon>
      <kvalitet>
        <Posisjonskvalitet>
          <datafangstmetode>2</datafangstmetode>
          <nøyaktighet>42</nøyaktighet>
          <synbarhet>2</synbarhet>
          <datafangstmetodeHøyde>2</datafangstmetodeHøyde>
          <nøyaktighetHøyde>42</nøyaktighetHøyde>
        </Posisjonskvalitet>
      </kvalitet>
      <senterlinje>
        <gml:LineString srsName="http://www.opengis.net/def/crs/epsg/0/5972" srsDimension="3">
          <gml:posList>568925,84 6661998,97 90,8 568920,52 6662017,57 90,8 568917,55 6662029,69 90,8 568911,79 6662034,72 90,8</gml:posList>
        </gml:LineString>
      </senterlinje>
      <opphav>
        <Opphav>
          <sikkerTerrenggjengivelse>true</sikkerTerrenggjengivelse>
          <datakilde>GenererteDataFraBildematching</datakilde>
        </Opphav>
      </opphav>
    </Terrenglinje>
  </wfs:member>
  <wfs:member>
    <Terrengpunkt gml:id="Terrengpunkt.1">
      <identifikasjon>
        <Identifikasjon>
          <lokalId>Terrengpunkt.1</lokalId>
          <navnerom>http://data.geonorge.no/SOSI/FKBHøydekurve</navnerom>
          <versjonId>Some meaningful text</versjonId>
        </Identifikasjon>
      </identifikasjon>
      <oppdateringsdato>2021-07-17T19:25:00Z</oppdateringsdato>
      <datafangstdato>2021-07-17</datafangstdato>
      <medium>T</medium>
      <verifiseringsdato>2021-07-17</verifiseringsdato>
      <registreringsversjon>2014-03-01</registreringsversjon>
      <informasjon>Some meaningful text</informasjon>
      <kvalitet>
        <Posisjonskvalitet>
          <datafangstmetode>2</datafangstmetode>
          <nøyaktighet>42</nøyaktighet>
          <synbarhet>2</synbarhet>
          <datafangstmetodeHøyde>2</datafangstmetodeHøyde>
          <nøyaktighetHøyde>42</nøyaktighetHøyde>
        </Posisjonskvalitet>
      </kvalitet>
      <posisjon>
        <gml:Point srsName="http://www.opengis.net/def/crs/epsg/0/5972" srsDimension="3">
          <gml:pos>568958,83 6662075,36 90,67</gml:pos>
        </gml:Point>
      </posisjon>
      <høyde>21.21</høyde>
      <opphav>
        <Opphav>
          <sikkerTerrenggjengivelse>true</sikkerTerrenggjengivelse>
          <datakilde>GenererteDataFraBildematching</datakilde>
        </Opphav>
      </opphav>
    </Terrengpunkt>
  </wfs:member>
</wfs:FeatureCollection>
